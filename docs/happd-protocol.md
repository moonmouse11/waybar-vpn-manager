...
Ограничения:
- Конфиг живёт, пока сервер не сменил ключи/порты; после смены сервера в GUI
  стоит перезахватить конфиги (ещё раз strace + экстрактор).
- Смена сервера внутри менеджера невозможна — выбор сервера остаётся за GUI.
- GUI, запущенный параллельно, корректно видит состояние (читает тот же happd):
  Connect/Disconnect в любом интерфейсе синхронизированы.
