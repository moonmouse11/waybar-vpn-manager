...
| `KeysProvider("vless")` / `KeysProvider("ss")` | `~/.config/vpn-manager/keys.json` (imported `vless://`/`ss://` keys) | happd process `xray-keys-*` + `keys-*-tun0` |
| `HappProvider` | n/a (GUI `/usr/bin/happ`, daemon `/tmp/happd.sock`) | `happ-*` interfaces + happd process list (minus `xray-keys-*`) |
